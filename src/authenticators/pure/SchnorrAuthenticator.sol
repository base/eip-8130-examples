...
/// @notice Schnorr signature authenticator over secp256k1 using the ecrecover precompile.
///
///         actorId = keccak256(parity (1) || px (32))  — 33-byte compressed public key
...
///
/// @dev Minimal transfer of `SchnorrVerifier` from base/eip-8130 @ 99d4331.
contract SchnorrAuthenticator is IAuthenticator {
    ...
    function authenticate(bytes32 hash, bytes calldata data) external pure returns (bytes32 actorId) {
        ...
        require(parity <= 1);
        actorId = keccak256(abi.encodePacked(parity, bytes32(pxVal)));
