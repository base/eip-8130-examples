...
/// @notice Groth16 ZK-SNARK authenticator over BN254 using EIP-196/197 precompiles.
///
...
///         verification key (VK) are passed in data and committed via actorId.
///
///         actorId = keccak256(vk || extraInputs)
///
...
///
///         NOTE: For production use, deploy per-circuit authenticators with hardcoded
///         VKs to avoid the calldata overhead of passing the VK every transaction.
///
/// @dev Minimal transfer of `Groth16Verifier` from base/eip-8130 @ 99d4331.
contract Groth16Authenticator is IAuthenticator {
    ...
    function authenticate(bytes32 hash, bytes calldata data) external view returns (bytes32 actorId) {
        ...
        actorId = keccak256(abi.encodePacked(data[vkOff:], data[257:vkOff]));
