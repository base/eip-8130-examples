...
/// @notice K1 multisig authenticator. Threshold M-of-N using secp256k1 ECDSA.
///
///         actorId = keccak256(threshold (1) || signerCount (1)
...
///
/// @dev Minimal transfer of `MultisigVerifier` from base/eip-8130 @ 99d4331.
contract MultisigAuthenticator is IAuthenticator {
    function authenticate(bytes32 hash, bytes calldata data) external pure returns (bytes32 actorId) {
        ...
        actorId = keccak256(data[:sigDataStart]);
