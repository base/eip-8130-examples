...
/// @notice Chainlink-priced payer authenticator for gas sponsorship.
///
...
///           - Max cost and gas limit come from the transaction-context precompile
///
///         Validation flow:
///           1. Derive actorId from the immutable TOKEN address
...
///
/// @dev Minimal transfer of `ChainlinkPayerVerifier` from base/eip-8130 @ 99d4331.
///      Cannot run against the shipped {ITransactionContext} precompile — see {IExtendedTransactionContext}.
contract ChainlinkPayerAuthenticator is IAuthenticator {
    ...
    bytes32 public immutable ACTOR_ID;
    ...
    constructor(
        ...
        ACTOR_ID = bytes32(uint256(uint160(token)));
    ...
    function authenticate(bytes32, bytes calldata) external view returns (bytes32) {
        ...
        _checkSender(requiredTokens);
        _checkPaymentPhase(requiredTokens);
    ...
    function _checkSender(uint256 requiredTokens) internal view {
    ...
    function _checkPaymentPhase(uint256 requiredTokens) internal view {
